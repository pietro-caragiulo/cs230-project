...
def main(unused_argv):
  ...
  # TEST 0 (success)
  # tf.summary.scalar("vz", feature_train_batch['vz'][0]) # works (but only one value per batch)

  # TEST 1 (fail)
  # tf.summary.image("vz", tf.reshape(feature_train_batch['vz'], [512, 1, 1, 1])) # Runs but values come with tag: vz/image0.. and they are not 512

  # TEST 2 (in progress)
  for i in range(512):
      tf.summary.scalar("vz_%d" % i, feature_train_batch['vz'][i])
